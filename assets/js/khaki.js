...
function _initAudioPlayer() {
    ...
    */
    ...
    var playlist = [];
    $('.nk-audio-playlist:not(.nk-audio-playlist-enabled)').addClass('nk-audio-playlist-enabled').each(function () {
        var _this = this;
...
/* Disqus */
function _initPluginDisqus() {

    // fix Disqus plugin position after resize inside equal-height row
    var $disqus = $('.row.equal-height #disqus_thread');
    if ($disqus.length) {
        var $equalBlocks = $disqus.parents('.row.equal-height:eq(0)').children();

        // check height of blocks
        setInterval(function () {
            var resized = 0;
            $equalBlocks.each(function () {
                if (!resized && this.clientHeight < this.scrollHeight) {
                    _debounceResize();
                    resized = 1;
                }
            });
...
var KHAKI = function () {
    ...
    _createClass(KHAKI, [{
        ...
        key: 'initPluginDisqus',
        value: function initPluginDisqus() {
            return _initPluginDisqus.call(this);
        }
    }, {
        key: 'initDoc',
